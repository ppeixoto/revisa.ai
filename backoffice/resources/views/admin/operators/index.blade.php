...
@section('breadcrumbs')
    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
        ...
        <li class="breadcrumb-item text-muted">Operadores</li>
    </ul>
@endsection

@section('buttons')
    @can('operators.create')
        <a href="{{ route('admin.operators.create') }}" class="btn btn-sm fw-bold btn-primary">Adicionar</a>
    @endcan
@endsection
            ...
            <div class="card">
                <div class="card-header border-0 pt-6">
                    <div class="card-title">
                        <div class="d-flex align-items-center position-relative my-1">
                            ...
                            <form action="{{ route('admin.operators.index') }}" method="get">
                ...
                <div class="card-body pt-0">
                    <div id="kt_table_admins_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                            ...
                            <table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer"
                                ...
                                <thead>
                                    <tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                                        ...
                                        @canany(['operators.edit', 'operators.destroy'])
                                                ...
                                                <div class="symbol symbol-circle symbol-50px overflow-hidden me-3">
                                                    <a href="{{ route('admin.operators.show', [$operator]) }}">
                                                        <div class="symbol-label fs-3 bg-light-danger text-danger">
                                                            {{ substr(ucfirst($operator->name), 0, 1) }}</div>
                                                    </a>
                                                </div>

                                                <div class="d-flex flex-column">
                                                    <a href="{{ route('admin.operators.show', [$operator]) }}"
                                                        class="text-gray-800 text-hover-primary mb-1">{{ $operator->name }}</a>
                                                    <span>{{ $operator->email }}</span>
                                                </div>
                                            </td>
                                            <td data-order="2021-04-15T20:43:00-03:00" class="text-uppercase">
                                                {{ $operator->roles->first()?->name }}</td>
                                            @canany(['operators.edit', 'operators.destroy'])
                        ...
                        <x-layouts.shared.pagination :items="$operators" />
